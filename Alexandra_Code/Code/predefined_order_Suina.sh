...
cd /home/projects/beyondMARS/Suina/
cat /home/projects/beyondMARS/data/Suina_ncbi.fasta | awk 'BEGIN {n_seq=0;} /^>/ {if(n_seq%1==0){file=sprintf("seq_%d.fa",n_seq);} print >> file; n_seq++; next;} { print >> file; }'


name=$(grep -oE '^>[a-zA-Z0-9_.]+' seq_2.fa) # The accession number
name="${name:1}"


# Construct vg graph from the chosen file and circularize it
/home/ctools/vg/bin/vg construct -M seq_2.fa > graph.vg
...
for d in {6,1,7,3,4,5,0}  # set the correct order 
do
